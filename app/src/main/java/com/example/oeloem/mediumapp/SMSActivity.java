...
public class SMSActivity extends AppCompatActivity {
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.btnsend:
                ...
                //Kondisi ketika permission diijinkan oleh pengguna (Marsmellow + )
                if (Build.VERSION.SDK_INT >= 23 && checkSelfPermission(Manifest.permission.SEND_SMS)
                        !=PackageManager.PERMISSION_GRANTED) {
                    //Menampilkan konfirmasi pada user untuk mengijinkan akses SEND_SMS
    ...
    //Penempatan method bisa dimana saja
